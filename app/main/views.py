...
def autocomplete():
    ...
    query = db.session.query(Product.name).filter(Product.name.like('%' + str(search) + '%'))
    results = [mv[0] for mv in query.all()]
    json = jsonify(matching_results=results)
    print(results)
    return render_template('autocomplete.html', matching_results=results)
